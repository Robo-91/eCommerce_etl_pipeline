...
def ingest_order_reviews():
    ...
    order_reviews_df = pd.read_csv(config['csv']['order_reviews'])
    ...
    truncate_query = 'TRUNCATE TABLE Order_Reviews_Staging'
    con.execute(text(truncate_query))

    # load data
    order_reviews_df.to_sql(
        'Order_Reviews_Staging',
        con = con,
        if_exists = 'append',
        index = False
    )
    ...
    # log results
    log_data = {'row_count': [len(order_reviews_df)], 'table_name': ['Order_Reviews_Staging'], 'load_date': [date.today()]}
